...
- ☐ Markup-independent behavioral contract layer (layer 2) — the layer that gates F1, where the DOM changes intentionally
- ☐ JSON Schema for meta + dev-mode validation, and the error-boundary/`onError` repair (§9.4, §2.6-3)
- ☐ Visual/a11y regression suite (mandatory for F1); note the 57 dangling `aria-describedby` references the DOM baseline records will trip it
