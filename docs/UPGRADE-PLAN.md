...
- Completed: automated baseline and critical-flow contracts (0.1–0.4); truthful callable-export public types plus the locked React 16/17/18 interop/CommonJS consumer matrix (0.6); asset deduplication, pack budgets and the packed-tarball consumer smoke, with source maps kept by decision (0.7); repo completeness for the imported input-integer fixtures (0.8).
- Partial: root Babel split landed but demo preset consolidation remains open (0.5); JS lint and CI are green while audit ownership and the devDependency sweep remain (0.9).
- Open: the remaining work within 0.5 and 0.9 listed above. Two Phase-0.7-scheduled decision gates were **not** decided here and stay open in §10: `engines.node >= 22` in the published manifest, and whether the four demo screenshots in `static/images` (~1.1 MB of the 7.25 MB) belong in the tarball at all — they are referenced only by demo markdowns, which are not published, but consumer meta could name any file in that folder.
