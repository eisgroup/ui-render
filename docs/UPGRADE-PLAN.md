...
| R18 | Tolerated lint warnings hide real defects in paths no test exercises | Medium | Medium–High | Confirmed once already: the `no-use-before-define` pair in `rules.js` was a temporal-dead-zone `ReferenceError` that silently swallowed any interpolated-template popup whose local data was the row array — 1920 green tests never reached it. Fixed with a reproducing regression test in `rules.popup-actions.test.js`, and `lint:js --max-warnings 0` now keeps the next one from being tolerated. Triage new warnings on the merits; suppress only with a comment stating why the rule is wrong |
