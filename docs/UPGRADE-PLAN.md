...
|---|---|---|---|---|
| `Table.js` (24 lines) | `Table` + `Header/HeaderCell/Row/Cell/Body/Footer` re-exports | `mapper.js` (incl. `Table.Cell` at :184), `TableView.js`, `LocalDraftTableRow.js`, `ErrorTable.js` | **Markup/className sugar only** over native `<table>` elements. Semantic's own table CSS is *not even loaded* (commented out in `_semantic.less`) — all table styling is already in-house LESS. | **S** |
...
- **The in-house table CSS depends on SUIR's className contract.** `src/style/components/table.less:125` hangs *every* table cell's padding off `.ui.table`, and SUIR's `Table` always emits `ui` and `table`. F1.1's conclusion ("visual parity near-guaranteed") holds, but its stated *reason* was wrong: parity is guaranteed only while the replacement keeps emitting those two tokens. `expand.less` keys off `.striped`/`.inverted` the same way.
- **Two unprotected DOM boundaries sit on this surface**, both latent only because no meta currently puts an engine prop there: `mapper.js:184` spreads a node's whole rest bag onto `Table.Cell`, and `TooltipPop.js:21` spreads its rest bag onto `Popup`. Steps 1 and 2 should apply `omitProps` in the new components, and `domProps.js`'s "WHERE THE LISTS ARE APPLIED" note should name both when they do.
...
- **Step 0 — contract audit + guard — SHIPPED.** Both deliverables landed; the audit's corrections to F1.1 are folded into the table above, and its findings per step are in the generated page.
  ...
  **The published parity checklist is `docs/SUPPORTED-PROPS.md`** (`scripts/generate-wrapper-prop-reference.js` + `scripts/wrapper-prop-curation.js`, guarded by `scripts/__tests__/wrapper-prop-reference.contract.test.js`, wired as `npm run docs:props` / `docs:props:check` in CI and `prepack`). It is a **separate page rather than part of `docs/SUPPORTED-VIEWS.md`** because prop names are not derivable from the `FIELD` constants that page is generated from — folding them in would have claimed a guarantee that generator cannot make. The two pages cross-link, and `docs:views:check` is untouched.
  ...
  The page keeps the **three outcomes distinct**, which is the part a naive audit gets wrong — "a prop appears in a meta" is not "a prop reaches SUIR". Each prop is **consumed** by the wrapper (we already own it; the swap cannot change it), **stripped** at the `domProps.js` boundary, or **forwarded** to SUIR (the actual parity risk). The forwarded set is then tiered: **37 tier-1 (wrappers + `Table` subcomponents) (across the three wrappers **and their six re-exported `Table` subcomponents**; the page tiers the 45 wrapper rows in three tables and lists the 16 subcomponent props in the consumer table above them)** props are exercised by a real meta or generated unconditionally by a wrapper; **24 tier-2** props are published propTypes/JSDoc that nothing in either corpus uses.
...
- **Step 1 — `Table` (S, ~2–3 days).** Native `<table>/<thead>/<tbody>/<tr>/<th>/<td>` implementation preserving the subcomponent API (`Table.Header`, `Table.Cell`, …) and the props Step 0 published. The exercised (tier-1) surface is **5 prop names across 7 components** — `children`, `className`, `style`, `colSpan`, `verticalAlign` — plus wrapper-only `fixedHeader`, which nothing passes. Removes the 4 KB SUIR table code path. Four obligations Step 0 added: keep emitting `ui table` on the root (the in-house cell padding selects on `.ui.table`); apply `omitProps` in the new `Table.Cell`; decide `verticalAlign` deliberately (SUIR emits `top aligned`, and no loaded CSS selects on `aligned`); and give the new `Table.js` its own `jest.config.js` per-file threshold, since only `TableView.js` has one today.
...
Run after Phase 2 (React 18) and Phase 3 (contract tests — they are the safety net). Steps 1–2 can proceed in parallel with Workstream A; Step 3 deserves dedicated focus. Every step is gated by the §9.5 behavioral layer **plus the mandatory visual/keyboard/a11y suite**. Total: **~4–7 weeks** spread across independently shippable releases — Step 0's audit left every per-step estimate unchanged (S / S–M / L), so the schedule stands; what it changed is *where* the effort sits in Steps 2 and 3 and what each step's definition of done includes. Completing 5a — including Step 3½ — removes the React 19 external blocker (§8).
...
- ☑ **Step 0:** passthrough-prop audit across example **and consumer** metas → published parity checklist at `docs/SUPPORTED-PROPS.md`, generated from the wrapper source and drift-checked in CI and in the suite. Result: 37 tier-1 (wrappers + `Table` subcomponents) (exercised) and 24 tier-2 (published, unexercised) forwarded props; `search`/`multiple`/`allowAdditions`/`clearable` are tier 2, `upward`/`disabled`/`colGroup` were found only in consumer metas
