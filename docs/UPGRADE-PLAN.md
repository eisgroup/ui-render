...
- **Visual/a11y regression is mandatory for F1** (not optional): Playwright over the demo with keyboard-navigation, focus, portal/click-outside and screen-reader-attribute assertions gates every F1 step; it stays optional for pure refactors, where DOM snapshots already gate. **Still does not exist, and since §9.7-F1 step 2 part 1 it BLOCKS A STEP'S COMPLETION rather than being carried as general debt** — the tooltip's whole reason for being (a bubble that appears next to its trigger, on top, unclipped, styled) is exactly the class of fact jsdom cannot express. The ceiling was measured, not assumed: **every `getBoundingClientRect()` in jsdom is 0×0**, so Popper computes `left: 0px; top: 0px` in every case, including one with a stubbed trigger rect. The named gaps, each one a test this leg owes and no jest suite can fake:

  | Gap | Why jsdom cannot | Consequence if left unwritten |
  |---|---|---|
  | Does the bubble land next to the trigger at all | no layout, so no rects | a replacement can position everything at the document origin and pass all 65 jest assertions |
  | Flip past a clipping ancestor | ditto — and flip is ACTIVE today (`enabled: !pinned`) | the one collision behaviour that is real parity is untested |
  | **The flip-changes-the-className contract** | the placement class carries Popper's *resolved* placement, but in jsdom it is always the requested one | the arrow and margins point the wrong way and nothing fails |
  | Whether the CSS applies at all | `Element.matches()` proves selector matching, never painted style | this is how the live styling defect (§9.7-F1 step 2, correction 3) went unnoticed; only computed style keeps a fix fixed |
  | Clipping | no overflow model | the corpus's own tooltips sit inside `Expand` → `AnimateHeight`'s `overflow: hidden`, so an inline rewrite fails here and only here |
  | Stacking | no paint order | `z-index` is `auto` today; whether the bubble covers host-page positioned content is unknown |
  | Real pointer semantics | `mouseEnter` is a synthetic dispatch; "the pointer travels across the gap into the bubble" is geometric | the `hoverable: false` defect and its fix are both unverifiable |
  | Touch | no touch model | on a `['click', 'hover']` overlay a tap fires **both** paths |
  | Real keyboard / AT | jsdom has no AT and no native focus semantics | whether a screen reader announces the text, and whether Escape reaches the document handler from inside a native control that consumes it |
  | Scroll/resize reposition and `hideOnScroll` | no scrolling | `eventsEnabled` is on today and its effect is invisible |
...
- **Step 2 — `TooltipPop` (S–M, ~3–5 days for the replacement). SPLIT IN TWO, AND PART 1 HAS SHIPPED.** In-house tooltip: portal + positioning + hover/focus triggers with delay + `aria-describedby`.
  ...
  **The positioning requirement, measured rather than inferred.** Flip is **active** today (`enabled: !pinned`, and the class carries Popper's *resolved* placement, so a flip is observable in the DOM). Overflow clamping is **off**: `preventOverflow` is `enabled: !!offset` and nothing sets `offset`. The arrow is a CSS `:before`, not a positioned element (`arrow` is `enabled: false`). Scroll/resize repositioning is on. So parity is **flip yes, shift no** — this step's claim that option (b) "would now be a regression … no flip, no overflow handling" is half right. Coordinates are unavoidable while the bubble portals out of the tree; an inline tooltip needs none, and the in-house `Tooltip.js` proves the pattern ships with an eight-placement class vocabulary already in `tooltip.less` — but at the corpus's own use sites an inline bubble would be clipped, because both declarations sit inside `Expand` → `AnimateHeight`, whose wrapper is `overflow: hidden` (and `.ui-render .app` is too; `progress.less` already carries a `// fix tooltip edge clipping` workaround). Neither library nor hand-rolled is ruled in; "no positioning code" and "inline only" are ruled out.
  ...
  **The gate part 2 will be judged against, and the part of it that does not exist.** The four suites above, all three React legs at identical totals, plus the §9.5 browser leg — which still does not exist and now blocks **this step's** completion, not only F1's. Everything positional is inexpressible in jsdom (every `getBoundingClientRect()` is 0×0), and the specific gaps are enumerated against the §9.5 Playwright item.
...
- ☐ **Step 2 part 2:** the replacement itself, and the positioning-primitive decision — deliberately still open, with the measured requirement (**flip yes, shift no**) recorded in §9.7-F1 step 2
...
- ☐ Visual/keyboard/a11y suite green per step (mandatory, §9.5) — **not met for Step 1**: the suite does not exist yet, so the `.ui.table` padding contract was verified by reading the compiled CSS and by class-string assertions rather than by rendered pixels. Carried as debt, not as done. **Now BLOCKING for Step 2** rather than debt: everything positional about a tooltip is inexpressible in jsdom (all rects are 0×0), so ten named gaps — including flip, the flip-changes-the-className contract, clipping, stacking, painted style, real pointer travel and touch — sit against the §9.5 item and cannot be closed by any jest suite. Step 2 part 1 wrote them down rather than faking them.
