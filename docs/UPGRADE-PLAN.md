...
**Visual/a11y regression is mandatory for F1** (not optional): Playwright over the demo with keyboard-navigation, focus, portal/click-outside and screen-reader-attribute assertions gates every F1 step; it stays optional for pure refactors, where DOM snapshots already gate. **SHIPPED 2026-09-01 by §9.7-F1 step 2 part 2, and BUILT FOR STEP 2** — `playwright.config.js` + `e2e/` (4 spec files / 39 tests / 1 shared fixture module / 1 reference ledger), gating CI job `browser`. It closes the ten gaps below for the tooltip. It is **not** retro-credited to Step 1 and does **not** pre-satisfy Step 3: Step 1's `.ui.table` padding contract was verified by reading compiled CSS and stays verified that way, and Step 3's `Dropdown` matrix is Step 3's work — what exists for it is the harness, the probes and a measured starting state (`e2e/reference.js` → `DROPDOWN`), which is the expensive half.
  ...
  **FIVE FINDINGS, three of which correct this document.** Full detail and every measured value in `e2e/reference.js`.
  ...
  |---|---|---|
  | Does the bubble land next to the trigger at all | no layout, so no rects | ✅ `harness` (adjacency on a ref-able trigger) + `corpus` (pinned at the broken value: centre distance > 500 px) |
  ...
  | **The flip-changes-the-className contract** | the placement class carries Popper's *resolved* placement, but in jsdom it is always the requested one | ✅ `harness` — `ui bottom left …` at the top edge, `ui top right …` at the right edge |
  ...
  | Real pointer semantics | `mouseEnter` is a synthetic dispatch; "the pointer travels across the gap into the bubble" is geometric | ✅ `corpus` — the `hoverable: false` defect, driven as real pointer travel |
  | Touch | no touch model | ✅ `tooltip.touch` under a `hasTouch` project — one tap fires both `on` paths |
  | Real keyboard / AT | jsdom has no AT and no native focus semantics | 🔶 `keyboard-a11y` — Tab reach, focus-does-not-open, Escape from inside a native input, `getByRole('tooltip')` empty. Real AT stays manual, permanently |
  | Scroll/resize reposition and `hideOnScroll` | no scrolling | ✅ `harness` — finding 3, isolated with a synthetic event |
  ...
  **What this leg still cannot see** — stated so the gate is not overclaimed, and carried forward as the remaining browser-level gap:
  ...
  - **Real OS pointer and touch.** `mouse.*` is CDP-synthesised and `hasTouch` is emulation; hover-on-touch heuristics and iOS Safari double-tap semantics are unverified.
...
- **Step 2 — `TooltipPop` (S–M, ~3–5 days for the replacement). SPLIT IN TWO, AND PART 1 HAS SHIPPED.** In-house tooltip: portal + positioning + hover/focus triggers with delay + `aria-describedby`.
  ...
  **Part 2 — the replacement, still open.** Positioning primitive options, unchanged and explicitly **not decided** — part 1 exists partly to give this decision its data:
  - **(a) `@floating-ui/dom`.** Few-KB, framework-agnostic, handles flip/shift/arrow; becomes the shared primitive for Step 3's option list as well.
  - (b) Zero-dep minimal positioning — the parity bar is lower than this step assumed (see below), so this is no longer obviously a regression.
  - (c) CSS anchor positioning — rejected for now (not Baseline across browsers).

  **The positioning requirement, measured rather than inferred.** Flip is **active** today (`enabled: !pinned`, and the class carries Popper's *resolved* placement, so a flip is observable in the DOM). Overflow clamping is **off**: `preventOverflow` is `enabled: !!offset` and nothing sets `offset`. The arrow is a CSS `:before`, not a positioned element (`arrow` is `enabled: false`). Scroll/resize repositioning is on. So parity looked like **flip yes, shift no** — this step's claim that option (b) "would now be a regression … no flip, no overflow handling" is half right. **The browser leg then corrected the correction:** flip also flips the alignment variation, so a right-edge trigger keeps the bubble on screen without `preventOverflow`; scroll repositioning is unreachable rather than merely invisible; and on every meta path popper writes no coordinates at all, so there is no positional behaviour to be a regression against (§9.5, findings 1–3). Coordinates are unavoidable while the bubble portals out of the tree; an inline tooltip needs none, and the in-house `Tooltip.js` proves the pattern ships with an eight-placement class vocabulary already in `tooltip.less` — **of which the browser leg measured only four to work; the four corner combinations, `top left` included, are broken (§9.5, finding 4)** — but at the corpus's own use sites an inline bubble would be clipped, because both declarations sit inside `Expand` → `AnimateHeight`, whose wrapper is `overflow: hidden` (and `.ui-render .app` is too; `progress.less` already carries a `// fix tooltip edge clipping` workaround). Neither library nor hand-rolled is ruled in; "no positioning code" and "inline only" are ruled out.

  **Decisions part 2 still owes, each now backed by a pinned test.** Apply `omitProps` — the boundary is unfiltered and `view`, `index` and `symbol` reach the bubble as HTML attributes today (`view="Tooltip"` on every `view: "Tooltip"` node), pinned as a defect rather than at zero so the fix is a visible diff. Decide `on`: SUIR's `['click', 'hover']` means every tooltip is also a click target and none opens on focus, and part 1 verified that `['hover', 'focus']` closes the keyboard gap immediately — a decision, not a workstream. Decide the passthrough: the reachable surface is `Popup.handledProps` ∪ `Portal.handledProps` = **45** props, 16 of them Portal-only and absent from `Popup`'s own propTypes, so a replacement accepting four narrows the meta contract by 41. Decide the trigger shape: `React.Children.only` means the `items` form of `view: "Tooltip"` has never worked — the engine catches the throw and renders its diagnostic in place of the node, so the author loses the trigger too (`docs/SUPPORTED-VIEWS.md` corrected in part 1). And delete the dead `TooltipPop` chain in `modules/form/utils.js` — 4 sites, confirmed unused — without touching line 7, which imports the *in-house* `Tooltip` as `ToolTip` and is live.

  **The gate part 2 will be judged against — now complete.** The four suites above, all three React legs at identical totals, plus the §9.5 browser leg, **which shipped 2026-09-01** (`playwright.config.js` + `e2e/`, 4 spec files / 39 tests, gating CI job `browser`). Same sequencing as part 1: measure and gate first, swap next, so the reference exists before the thing it judges. Every browser-only fact lives in `e2e/reference.js` tagged `[R]` reference / `[I]` invariant / `[R->I]` pinned defect, and the diff to that file is what part 2's review reads — a suite that failed wholesale on the planned replacement would be a tripwire on the plan, not a gate.

  **Five things the browser leg found, and the four claims above it corrects.** Detail in §9.5; the short version, because they change what part 2 is *for*:
  1. **The tooltip that ships is unpositioned, not merely unstyled, and it throws once per open.** No coordinates are ever written from any meta path, because nothing a meta can declare can hold a `ref` and popper's reference element is therefore `null`. Converging on the inline `Tooltip` is a strict improvement over a bubble that lands ~700–3000 px from its trigger, so the risk register for part 2 shrinks to one item: **clipping**. Knowingly left unfixed here — the fix is one `React.forwardRef` on `Button`, or `mapper.js` using the already-present `RowRef` — because this branch's mandate was reference-only and part 2 deletes the code path; recorded rather than silently carried, since it is a user-visible defect surviving a release.
  2. **"Flip yes, shift no" understates it.** Flip flips the alignment variation too, so a right-edge trigger keeps the bubble on screen without `preventOverflow`. And popper writes **no `data-popper-*` attribute**, so the resolved placement is readable only from the className.
  3. **"`eventsEnabled` is on and its effect is invisible" is right for the wrong reason.** Any scroll anywhere closes the bubble (and it flickers back ~50 ms later), because SUIR 3.0.0-beta.2 never destructures the `hideOnScroll` prop and its own local function of that name makes the scroll `EventStack` unconditional. The repositioning is unreachable, so part 2 owes nothing for it.
  4. **The vocabulary this step counted on is half broken.** `tooltip.less`'s eight placement combinations are four working ones (`top`, `bottom`, `left`, `right`) and four broken corners: `top left` / `top right` land **on** the host, `bottom left` / `bottom right` degrade to `left` / `right`. `top left` is the only placement `TooltipPop` uses, so **part 2 owes a `tooltip.less` fix or an explicit decision to restrict the vocabulary** — a new obligation this step did not have before, and the single most consequential thing the leg produced.
  5. **Clipping is demonstrated, and the shape that survives it is now known.** `.app` is `position: relative; overflow: hidden` with no transform/filter/will-change, so `position: fixed` inside `.ui-render` is viewport-relative and escapes the clip while still getting the 13 scoped rules — the combination this step wanted. And `.ui-render` creates no stacking context, so the only paint-order lever is the bubble's own `z-index`: the inline component's `9` beats host content at `6`, the portal's `auto` does not.
...
- ☑ **Step 2 part 1:** the className contract pinned as a literal (`ui top left inverted popup transition visible`) with the 13 scoped CSS rules and 6 arrow rules it feeds named against it
- ☑ **Step 2 part 1:** the interaction contract pinned as behaviour and exact milliseconds — 500 open / 70 close, leave-before-500 cancels, click opens instantly, second click / click-outside / Escape close, click inside does not, pointer-onto-bubble still closes
...
- ☐ **Step 2 part 2:** the popper crash left **knowingly unfixed** — one `React.forwardRef` on `Button`, or `mapper.js` using the already-present `RowRef`. Pinned rather than fixed because the browser branch's mandate was reference-only and the swap deletes the code path; recorded here because it is a user-visible defect surviving a release
- ☐ **Step 2 part 2:** the replacement itself, and the positioning-primitive decision — deliberately still open. The measured requirement is **no longer "flip yes, shift no"**: the browser leg found that flip also flips the ALIGNMENT, so horizontal overflow is already handled, while scroll repositioning is unreachable (SUIR's unconditional `hideOnScroll`) and no coordinates are written at all on any meta path. Recorded in §9.7-F1 step 2 and measured in `e2e/reference.js`
