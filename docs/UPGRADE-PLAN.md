...
| 0.9 | Lint & security baseline | **Completed.** `lint:js` now exits **0 errors / 0 warnings** and carries `--max-warnings 0`, so a new warning fails CI. All 22 warnings were triaged individually, not blanket-silenced: 17 were genuine cleanups (dead destructures and imports, a misplaced `eslint-disable` that silenced nothing, explicit `return undefined` in two sentinel getters, a useless escape and rename); 5 are documented suppressions where the rule is wrong or the fix is riskier than the warning (`alt` arrives through a spread, two `exhaustive-deps` guard reads that would clobber user input if listed, a `switch` that passes unknown types through, and the `no-loop-func` renderer closure). The `no-use-before-define` pair was a **real defect** — see §11 R18. Audit baselines re-measured on 2026-08-10: `npm audit --omit=dev` **0**, full audit **26** (2 critical, 12 high — all dev tooling; was 20 on 2026-07-21, and the burn-down owner is still an H9 governance item). All **14** zero-reference devDependencies removed (the 13 plus the unverified `dot-prop-immutable`), dropping the installed dev tree from 1443 to 1429 packages. |
...
- `src/style/unused/` (10 files); `override/_policy.less` / `_classic.less` (verify unreferenced); icomoon demo artifacts under `fonts/icons/`; the unreferenced engine `tester/` fixtures. **The `tester/` pair is now verified deletable and worse than merely dead:** `test_data.js` and `test_meta.js` re-export from `../examples/…`, and no `examples/` directory exists under `src/core/pages/main/` (the fixtures live in `src/demo/examples/`), so either file would fail module resolution if anything imported it. Nothing does — the repo-wide grep finds no importer. `eslint-config-react-app` cannot see this because `import/no-unresolved` is off. Deleting both also removes two permanently-0% files from the coverage report.
...
| R18 | Tolerated lint warnings hide real defects in paths no test exercises | Medium | Medium–High | The 0.9 triage confirmed it four times over. **Fixed:** the `no-use-before-define` pair in `rules.js` was a temporal-dead-zone `ReferenceError` that silently swallowed any interpolated-template popup whose local data was the row array — 1920 green tests never reached it (regression test now in `rules.popup-actions.test.js`); the `no-useless-escape` line in `demo/services/axios.js` indexed a possibly-null `String.match`, so an unmatched server message threw a TypeError inside the rejection handler and replaced the real API error; and a `no-unused-vars` import exposed a JsonView test whose name promised a click it never performed (renamed; the real toggle coverage lives in `JsonView.behavior.test.js`). **Recorded, not fixed:** `hasObjKeys(…, 'shallow')` in `utils/object.js` evaluates its loose-equality clause even for object-like values, so a shallow object match the first clause accepted is then rejected by reference inequality — reachable through the two `hasObjKeys` callers in that file; needs restructuring plus a test, so it is not lint-pass work. `lint:js --max-warnings 0` now stops the next one from being tolerated: triage on the merits, and suppress only with a comment stating why the rule is wrong |
...
- ✅ 0.9: `lint:js` exits 0 errors / 0 warnings behind `--max-warnings 0`; all 22 warnings triaged (17 fixed, 5 suppressed with stated reasons, 1 real defect fixed with a regression test — R18); audit baselines re-measured (prod 0, full 26); all 14 zero-reference devDependencies removed. Assigning the audit burn-down owner remains H9 governance
