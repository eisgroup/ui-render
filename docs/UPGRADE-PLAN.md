...
1. **`semantic-ui-react` will be exited entirely** (§9.7-F1). The audited dependency surface is far smaller than the package's reputation suggests: exactly **3 wrapper components** (`Table` — **now in-house, step 1 shipped**; `TooltipPop`; `Dropdown`) and **5 curated LESS modules** — the codebase has already been trending out of it (slider lib and react-dropzone removed recently, modal and pagination already in-house). Completing the exit also removes the main external React 19 blocker.
...
- **The two non-leak markup defects the baseline recorded — fixed 2026-08-28** (`components/Expand.js`, `pages/main/components/TableView.js`; the sticky-class contract in `pages/main/components/__tests__/TableView.test.js`, zero-tripwires in `FIXED_MARKUP_JUNK`). `Expand` rendered `id={String(id)}` while `id` is optional in meta, so `String(undefined)` shipped the literal attribute `id="undefined"` — repeated on every such view in a document, which means a `<label for>` could only ever resolve to the first. **The second was ten times larger than the ledger measured:** `TableView` appended `-last` to every cell whose right-hand neighbour is not pinned, but `-last` is only meaningful on `sticky` (`table.less` draws `td.sticky-last::after`). The ledger counted the `undefined-last` spelling — 20, where `className` was `undefined` — and missed the bare `-last` class produced when `className` was the empty string: 190 more, 210 in total. Counting one spelling of a defect measures the spelling, not the defect. **Two things worth carrying forward.** (1) Removing `-last` traded 190 junk classes for 190 inert `class=""`: Semantic's `Table.Cell` ran its own `cx()` and emitted the attribute whatever we passed, so suppressing it from outside was dead code — measured, and the note was in the source. **Discharged by F1 step 1**, and by more than the 190: the in-house cell omits an empty `className`, which removed all 317 empty class attributes in the baseline (24 `tbody`, 94 `tr`, 199 `td`) — every one of them Semantic's, none of them ours. (2) The 38-example corpus contains **zero** `sticky` classes, so it could neither catch this bug nor prove the fix preserves the feature; that is covered by a direct unit test instead. This is the third time the corpus proved to be a regression net rather than an audit — see also the seven DOM boundaries below and the `index=` miscount. **The ledger is now empty**, and kept as the place a new defect goes rather than deleted.
...
- **CSS ↔ markup join — shipped by §9.7-F1 step 2 part 1** (`src/style/__tests__/css.tooltip-contract.test.js`, 8 tests): compiles the final CSS through the same webpack PostCSS stage, renders the real open tooltip, and runs `Element.matches()` for every rule against the real node. The two existing CSS suites assert what the stylesheet *contains* and the component suites assert what the DOM *is*; neither could see whether any rule selects the node that ships, and that gap hid a live styling defect — the tooltip's 13 scoped rules match nothing, because the portal mounts outside `.ui-render`. Pinned in both directions, so fixing it is deliberate and regressing it is loud. **Worth generalising:** any component that portals out of the widget has the same exposure, and the modal `Popup` only escapes it because `#render-popup-root` lives inside `.ui-render`.
- - **What the reference measured about the planned convergence — the specification for step 2's second half.** The browser leg was run against a *simulated* convergence (`TooltipPop` rendering the in-house inline `Tooltip`) before any replacement was written, and the failures are the obligations, not a verdict on the suite. Three are real behaviour the inline component does not have, because it reveals through the CSS rule `*:hover > &` in `src/style/components/tooltip.less` and has no JavaScript at all: **the 500 ms open delay disappears** (a deliberate UX decision recorded in `TooltipPop.js` — "avoiding an accidental popup when the user is already familiar with the UI"), **click-to-open disappears**, and **Escape and click-outside stop dismissing it** — the last two are accessibility, not polish. A fourth is a hazard the stylesheet only hints at (`tooltip.less:12` comments that `visibility: hidden` "will trigger hover"): an inline bubble positioned over its trigger **intercepts pointer events**, which made hovering the trigger impossible — `pointer-events: none` is therefore a requirement, and the reason `Slider`/`Upload` do not hit it is that their bubbles do not cover an active area. So the convergence is *not* "delete the wrapper and render `Tooltip`": it keeps the delay and the dismissal paths in JavaScript and leaves CSS to draw. Two further failures (`both declarations render their own text`, and the adjacency assertion) are unexplained and left open deliberately — they are cheap to diagnose against a real implementation and expensive to chase against a straw one.
- **A method note, because it cost four iterations.** The same five failures were "diagnosed" three times from the aggregate count — a coupled selector, presence-versus-visibility, pointer interception — and each hypothesis was individually right yet moved the number barely or not at all, because several causes overlapped. Reading the failure text of ONE test found the real blocker in its first line, and the largest remaining cause turned out to be in the `open`/`openByHover` helpers, which waited on the portal selector themselves so tests failed *before* their own assertions. Grepping test bodies for the selector missed them. For a multi-cause failure, the aggregate count is a bad instrument: read one failure, fix it, repeat.
**Visual/a11y regression is mandatory for F1** (not optional): Playwright over the demo with keyboard-navigation, focus, portal/click-outside and screen-reader-attribute assertions gates every F1 step; it stays optional for pure refactors, where DOM snapshots already gate. **SHIPPED 2026-09-01 by §9.7-F1 step 2 part 2, and BUILT FOR STEP 2** — `playwright.config.js` + `e2e/` (4 spec files / 39 tests / 1 shared fixture module / 1 reference ledger), gating CI job `browser`. It closes the ten gaps below for the tooltip. It is **not** retro-credited to Step 1 and does **not** pre-satisfy Step 3: Step 1's `.ui.table` padding contract was verified by reading compiled CSS and stays verified that way, and Step 3's `Dropdown` matrix is Step 3's work — what exists for it is the harness, the probes and a measured starting state (`e2e/reference.js` → `DROPDOWN`), which is the expensive half.

  **What it is, mechanically.** `@playwright/test` pinned exactly in `devDependencies` (`1.62.1`); `npm run test:e2e:install` downloads Chromium once per machine and is deliberately **not** a `postinstall` hook, so `npm ci` stays exactly as fast as it is today for everyone who never runs this leg. It serves a **production build** rather than `npm start`, because opening a tooltip currently throws and webpack-dev-server turns an unhandled error into a full-viewport overlay iframe that intercepts every subsequent interaction — disabling that overlay would take a real debugging affordance away from humans to suit a test runner. Two env overrides in `webpack.demo.config.mjs` (`PUBLIC_PATH`, `OUTPUT_DIR`, both defaulting to today's values) make the build root-relative into `build-e2e/` so `serve --single` can host it; that is also why `npm run serve-build` cannot currently serve `/examples` at all. Spec files are named `*.pw.js` **and** `e2e/` is in `testPathIgnorePatterns`, because the repo root is a jest `roots` entry and jest's default `testMatch` would otherwise collect them; the legacy React legs inherit that entry by spreading `jest.config.js`. Cold run, measured locally: **68 s** including the build. Chromium cache, measured: **567 MB on disk** (368 MB full browser + 196 MB headless shell + 2.5 MB ffmpeg); `--only-shell` would cut it to ~196 MB and is deliberately not used, because a CI/local browser split is exactly what makes a browser gate unreproducible.

  **How a tooltip became observable at all.** Part 1 measured that no tracked example rendered a `TooltipPop`: the only `view: 'Tooltip'` and the only `tooltip:` both sit in the `Factors` tab of `_meta.js`, an inactive `Tabs` panel. Three fixes were costed and two rejected. **Activating the branch** (`_meta.js:277`) was rejected — it rewrites ~240 snapshot lines, moves `ROLE_CENSUS.all`, and changes what the public demo shows, to save one click. **Adding an example** was rejected at **nine enforced touchpoints** (`manifest.contract` length, `Examples.registry-and-rendering` length, the 38-snapshot baseline, `ROLE_CENSUS`, `FORM_BINDINGS`, `NAMELESS_CONTROLS`, `BOUND_NAME_ATTRIBUTES`, `META_ATTRIBUTES` + the generated prop page). **Adopted:** the existing branch is driven as-is — `/examples#all` auto-expands from the hash and one click on the `Factors` tab mounts the panel — **plus** one `tooltip:` line on `src/demo/examples/button-icon_meta.js`, so the leg does not depend solely on a 749-line meta and a tab click. That line is **measured free**: a closed tooltip adds nothing to the document, and `META_ATTRIBUTES` is keyed by *wrapped* view (Table/Tooltip/Select/Dropdown), which `Button` is not. All three React legs stayed at 160 suites / 2451 tests / 38 snapshots. The deep site is still driven, because clipping is measurable only there — its trigger has **ten** clip/scroll ancestors.

  **The one thing the corpus cannot give: a ref-able trigger.** `src/demo/pages/TooltipHarness.jsx` is an unlisted route (`/harness/tooltip?section=…`, deliberately absent from `NavTabs`) rendering `TooltipPop` on plain `<button>`s. It is not a convenience — see finding 1 below. It sits under `src/demo/` so it costs nothing against `collectCoverageFrom` (`src/core/**` + `src/library/**`) and never reaches the npm tarball (`package.json#files`); `lint:js` does cover it. One section per page load, because the first version put every section on one scrolling page and popper then computed coordinates **124 px** off — a nested-scroll-container artefact of the instrument that would have been recorded as a fact about the product.

  **FIVE FINDINGS, three of which correct this document.** Full detail and every measured value in `e2e/reference.js`.

  1. **The shipped tooltip is not merely unstyled — it is unpositioned, and it throws.** At every use site reachable from meta, popper's wrapper keeps `left: 0px; top: 0px` with `transform: none` (no coordinates are ever written) and each open raises exactly one uncaught `TypeError: Failed to execute 'getComputedStyle' on 'Window'` from the clipping-parent lookup inside the **flip** modifier. Cause: SUIR clones the trigger with a `ref`, and **nothing a meta can declare can hold one** — `mapper.js` uses the plain `Row`, `Button` is `React.memo(function Button)`, and the only ref-able components in `src/core` are `Dropzone` and the named `RowRef` export, which no `view` maps to. Feeding the same component a plain `<button>` positions correctly and raises nothing, which is what makes this a diagnosis rather than a guess. **Correction:** the sentence below that jsdom is useless "because Popper computes `left: 0px; top: 0px` in every case" is right about jsdom and wrong about the implication — real Chrome computes the same thing at the real use sites, so the naive browser assertion proves nothing jsdom did not. What discriminates is whether coordinates were written *at all*, proximity to the trigger, and the page-error count. **Knowingly left unfixed on that branch** (its mandate was reference-only, and part 2 deletes the code path): the fix is one `React.forwardRef` on `Button`, or `mapper.js` using the already-present `RowRef`. Recorded here rather than silently carried, because it is a user-visible defect surviving a release.
  2. **"Flip is active and observable" is true of the config and false of the running product** — flip is the modifier that crashes. With a ref-able trigger it works on **both** axes: at the top edge `top left` resolves to `bottom left`, at the right edge to `top right`, which keeps the bubble inside the viewport. **Correction:** §9.7-F1 step 2's "flip yes, shift no" understates the parity bar — horizontal overflow *is* handled, by flip changing the alignment variation rather than by `preventOverflow`. Also measured: popper writes **no `data-popper-*` attribute at all**, even when positioning correctly, so the resolved placement is readable only from the className — which is exactly why the flip contract is a class assertion.
  3. **Every scroll anywhere in the document closes the bubble.** Not `closeOnTriggerMouseLeave`: a *synthetic* `window` `scroll` event with no pointer movement and no layout change closes it, and it reopens ~50 ms later — a flicker. Upstream cause, read out of the installed source: `Popup.js` never destructures the `hideOnScroll` **prop**, so its own local `var hideOnScroll = function…` shadows it and `hideOnScroll && <EventStack name="scroll" target="window"/>` is always truthy; `@semantic-ui-react/event-stack` subscribes with `capture: true`, so a nested container's non-bubbling scroll reaches it too. **Correction:** "`eventsEnabled` is on today and its effect is invisible" is right for the wrong reason — the repositioning is not merely invisible, it is unreachable, so part 2 owes nothing for it.
  4. **The convergence target has its own positioning defect, in the exact placement the replacement would use.** This document credits `tooltip.less` with an eight-placement vocabulary the in-house `Tooltip` brings with it. Measured: **four of eight work.** `top`, `bottom`, `left`, `right` place the bubble clear of its host; `top left` and `top right` land **on top of** the host, and `bottom left` / `bottom right` silently degrade to plain `left` / `right`. Cause: `.tooltip.left` / `.tooltip.right` set `top: 50%` at the same specificity as `.tooltip.top` / `.tooltip.bottom`, so a corner class string matches both and the axis rule loses — over-constrained for `top left`, on source order for `bottom left`. **`top left` is the only placement `TooltipPop` uses in production**, so converging on `Tooltip` without touching `tooltip.less` would ship a bubble sitting on its own trigger. Invisible to jsdom, which resolves neither the cascade nor over-constrained absolute positioning. Part 2 owes a `tooltip.less` fix or an explicit decision to restrict the vocabulary.
  5. **Clipping is the one real cost of going inline, and it is now demonstrated rather than predicted.** In one 220×56 `overflow: hidden` box the portaled bubble renders 9 px above the box's top edge and is hit-testable there; the inline bubble overflows by 66 px and is **not** hit-testable outside it. Both halves in the same box on the same page. Related, and the open question part 2 needed answered: `.app` is `position: relative; overflow: hidden` with **no** transform/filter/will-change, so `position: fixed` inside `.ui-render` *is* viewport-relative and escapes the clip — a fixed bubble mounted inside the widget gets the scoped CSS *and* avoids the clip. And `.ui-render` is `position: static; z-index: auto; isolation: auto`, i.e. the widget creates **no stacking context of its own**, so paint order against host content is decided by the host and the only lever is the bubble's own `z-index`: the inline `Tooltip`'s `9` beats host content at `6`, the portal's `auto` does not. That is a concrete argument *for* the inline component.

  The ceiling that made this leg necessary was measured, not assumed: **every `getBoundingClientRect()` in jsdom is 0×0**, so Popper computes `left: 0px; top: 0px` in every case, including one with a stubbed trigger rect. The named gaps, and which spec now closes each:

  | Gap | Why jsdom cannot | Closed by |
  |---|---|---|
  | Does the bubble land next to the trigger at all | no layout, so no rects | ✅ `harness` (adjacency, centred on the trigger) + `corpus` (centre distance < 120 px, measured 40 px at both use sites — was pinned at the broken value, > 500 px) |
  | Flip past a clipping ancestor | ditto — and flip is ACTIVE today (`enabled: !pinned`) | ✅ `harness` — and finding 2: flip is also what crashes on the meta path |
  | **The flip-changes-the-className contract** | the placement class carried Popper's *resolved* placement, but in jsdom it is always the requested one | ✅ `harness` — and part 3 RESOLVED it by removal: CSS placement cannot flip, so the requested placement is the one you get. Now pinned as the loss (`no flip`, `no shift`), reachable only here and never from a meta |
  | Whether the CSS applies at all | `Element.matches()` proves selector matching, never painted style | ✅ `corpus` — computed `background`/`border`/`padding`/`box-shadow`/`max-width`/`z-index`/`::before` on the node that ships |
  | Clipping | no overflow model | ✅ `harness` — finding 5, both directions in one box |
  | Stacking | no paint order | ✅ `harness` — `elementFromPoint`, no pixel baseline involved |
  | Real pointer semantics | `mouseEnter` is a synthetic dispatch; "the pointer travels across the gap into the bubble" is geometric | ✅ `corpus` — still closes, and part 3 established that it CANNOT do otherwise: the bubble keeps `pointer-events: none`, so the pointer over it is really over what is behind it. That also invalidated `elementFromPoint` as a paint probe (`paintedTopmostAt` exists for that) |
  | Touch | no touch model | ✅ `tooltip.touch` under a `hasTouch` project — was "one tap fires both `on` paths", now "a tap opens nothing" |
| A hover that a TOUCH pointer produced | **jsdom can, with care** — it implements no `PointerEvent`, so RTL falls back to `MouseEvent`, `pointerType` is not an init key, and `fireEvent.pointerOver(el, {pointerType})` delivers `null`; defining the property on a hand-built event does reach React | ✅ both legs. `tooltip.touch` is what FOUND it (the bubble appeared 500 ms after every tap, because the tap's compatibility mouse sequence leaves a sticky emulated hover), and `TooltipPop.behavior` pins it plus the fail-open — an unknown pointer type must still open on hover |
| **A focus caused by a pointer** | nothing in jsdom focuses on click, so the case cannot arise there | ✅ `harness` + `corpus`. Clicking a `<button>` focuses it, and focus-open then showed the bubble instantly — so dropping the click gesture changed nothing a user sees until this was suppressed too |
| **A wrapper element stretched by its flex container** | no layout, so no blockification and no `align-items: stretch` | ✅ `harness` + `corpus`. `.tooltip-host` is `inline-flex`, but a flex item is blockified to `flex` and then stretched: inside `div.flex--col` the host came out 1222 px wide around a 35 px button and the bubble followed it, 559 px away, with every jsdom suite green |
  | Real keyboard / AT | jsdom has no AT and no native focus semantics | 🔶 `keyboard-a11y` — Tab reach, focus-DOES-open, Escape from inside a native input, `getByRole('tooltip')` = 1. Real AT stays manual, permanently |
  | Scroll/resize reposition and `hideOnScroll` | no scrolling | ✅ `harness` — finding 3, isolated with a synthetic event. Part 3 resolved it by removal: `hideOnScroll` is dropped and a scroll no longer closes the bubble, because a bubble that is a child of the thing that moves needs no repositioning |
  ...
  **What this leg still cannot see** — stated so the gate is not overclaimed, and carried forward as the remaining browser-level gap:
  ...
  - **Cross-browser.** Chromium only, which is narrower than `browserslist` (`>0.2%, not dead`). WebKit and Firefox cost a measured 102 MiB and 108 MiB of download plus their own flake.
  ...
  - **The peer matrix.** The demo builds against the installed React 18 only; this leg does not extend browser coverage to 16.14 or 17.
  - **`prefers-reduced-motion`, forced-colors, zoom, RTL** — not emulated.
  - **`tabs` / `upload` keyboard contracts.** Layer (2) records that both expose no interactive role at all, and §9.5 assigns them here; this leg does **not** cover them. `Tabs.js`'s `.tabs__item` divs carry `onClick` and neither `role` nor `tabindex`, so they are unreachable by keyboard — a defect awaiting its own step, not something this leg silently fixed.

  The same ceiling applies to `Dropdown` at step 3 (`upward`'s auto-flip is the identical class of fact) and to `tabs`/`upload`, which layer (2) already records as having no interactive role to drive.
...
|---|---|---|---|---|
| ~~`Table.js`~~ **DONE (step 1)** | was `Table` + `Header/HeaderCell/Row/Cell/Body/Footer` re-exports | `mapper.js` (incl. `Table.Cell` at :184), `TableView.js`, `LocalDraftTableRow.js`, `ErrorTable.js` — all unchanged, the subcomponent API survived the swap | **Markup/className sugar only** over native `<table>` elements, which is exactly what it turned out to be: 24 lines of wrapper became 119 lines of seven components, no new dependency. Semantic's own table CSS is *not even loaded* (commented out in `_semantic.less`) — all table styling was already in-house LESS, so nothing about styling changed. | **S**, spent |
| `TooltipPop.js` (24 lines) | `Popup` | `mapper.js` (`Render.Tooltip` at :44, tooltip renderer at :499), plus the `tooltip` attribute on **any** node via `Render.js:107–111` | Hover tooltip: `trigger`/`content`, 500 ms enter delay, `inverted`, position passthrough. | **S–M** (needs a positioning primitive) |
| `Dropdown.js` (291 lines) | `Dropdown` | `mapper.js` Dropdown view (:560), **and `modules/form/inputs/DropdownField.js` for every `view: "Select"`** — the majority path | The real work: `selection`, `lazyLoad`, `upward`, `disabled`, `compact`, `noResultsMessage` (generated but **inert** — SUIR renders it only under `search`, which no meta in either corpus enables, so do not port it as a requirement), keyboard navigation, cascading reset. Note: the wrapper already **owns the external API** — `onChange(value, name, event)`, option sanitization, case-insensitive dedup, cascading reset all live in wrapper code and are keepers. | **L** |

**Step 0 corrected four things in the table above** (see `docs/SUPPORTED-PROPS.md` for the derived, CI-checked version of all of it):

1. **`search`/`multiple`/`allowAdditions`/`clearable` are used by nothing.** Zero occurrences on any `Select`/`Dropdown` node in the 38 tracked examples and zero in the consumer metas. They were named here as "the real work"; they are not. The **L stands**, but it sits in the keyboard/a11y matrix, the cascading flows, `upward`'s auto-flip and the `.ui.selection.dropdown` CSS contract — and whether to reimplement or deprecate the unexercised half is now an explicit owners' call, not an assumed cost.
2. **Three props are used only by consumer metas** — `upward` and `disabled` on `Select`, `colGroup` on `Table`. The first two reach SUIR and are both styled, so a checklist derived from the demo corpus alone would have been wrong.
3. **The `Dropdown` consumer list was incomplete**: `DropdownField.js` was missing, and it is the majority path. It matters because it imports the **named** export — `React.memo(...)` does not carry `displayName`, so the `form/utils.js:193,199` branch works only on that import. Step 3 must preserve the split, not just the string; a contract test now pins both.
4. **`TooltipPop` has a second entry point**: the `tooltip` attribute on any node (`Render.js`). When `tooltip` is an object it is spread straight into SUIR, so it is an unfiltered passthrough — a wider surface than "the tooltip renderer in `mapper.js`".

**Two findings that change the work rather than the estimate:**

- **The in-house table CSS depends on SUIR's className contract.** `src/style/components/table.less:125` hangs *every* table cell's padding off `.ui.table`, and SUIR's `Table` always emitted `ui` and `table`. F1.1's conclusion ("visual parity near-guaranteed") holds, but its stated *reason* was wrong: parity is guaranteed only while the replacement keeps emitting those two tokens. `expand.less` keys off `.striped`/`.inverted` the same way. **Honoured by step 1**: the in-house root composes `ui <modifiers> table <className>` in that order, all 24 `<table>` class strings in the DOM baseline are byte-identical, and both a unit test and the baseline pin it. `inverted`/`striped` survived as props for the same reason — unlike `celled`, they *are* styled.
- **Two unprotected DOM boundaries sit on this surface**, both latent only because no meta currently puts an engine prop there: `mapper.js:184` spreads a node's whole rest bag onto `Table.Cell`, and `TooltipPop.js:21` spreads its rest bag onto `Popup`. Steps 1 and 2 should apply `omitProps` in the new components, and `domProps.js`'s "WHERE THE LISTS ARE APPLIED" note should name both when they do. **The first is closed (step 1):** all seven table components apply both lists, `domProps.js` names the family, and the DOM baseline is unchanged by it — a zero-diff net, since no tracked example puts an engine prop on a `TableCells` node. The tooltip half is still open and belongs to step 2.
...
- **Step 0 — contract audit + guard — SHIPPED.** Both deliverables landed; the audit's corrections to F1.1 are folded into the table above, and its findings per step are in the generated page.

  **The published parity checklist is `docs/SUPPORTED-PROPS.md`** (`scripts/generate-wrapper-prop-reference.js` + `scripts/wrapper-prop-curation.js`, guarded by `scripts/__tests__/wrapper-prop-reference.contract.test.js`, wired as `npm run docs:props` / `docs:props:check` in CI and `prepack`). It is a **separate page rather than part of `docs/SUPPORTED-VIEWS.md`** because prop names are not derivable from the `FIELD` constants that page is generated from — folding them in would have claimed a guarantee that generator cannot make. The two pages cross-link, and `docs:views:check` is untouched.

  Following the §9.4 pattern, the page splits derived from curated and enforces the seam. **Derived** (never curated): every file that reaches SUIR by `import`, `require` *or* `jest.mock`; each wrapper's own destructured props and line count; which `domProps.js` lists each wrapper applies; the JSX attributes written on the SUIR element, the props the wrapper generates onto the rest bag, and whether the rest spread lands before or after them (which decides who wins a collision); and every attribute the codebase puts on `Table` and each re-exported subcomponent. **Curated**: what a prop means, and whether any real meta uses it. **Enforced by the contract test**: the drift gate; the parsed `domProps.js` lists against the real exported arrays; the tracked example corpus walked from the `EXAMPLES` manifest and compared attribute-for-attribute; the `displayName` split above; and the eslint guard still being configured.

  The page keeps the **outcomes distinct**, which is the part a naive audit gets wrong — "a prop appears in a meta" is not "a prop reaches SUIR". Each prop is **consumed** by the component (we already own it; the swap cannot change it), **stripped** at the `domProps.js` boundary, **forwarded** to SUIR (the actual parity risk), or — once that component's step has landed — **dropped**, which is the fourth outcome step 1 added and the semver record for the step. The forwarded set is tiered: at step 0 it was **37 tier-1** (exercised by a real meta or generated unconditionally by a wrapper) and **24 tier-2** (published propTypes/JSDoc that nothing in either corpus uses), across the three wrappers and the six re-exported `Table` subcomponents. Since step 2 it is **17 / 13**: the `Table` family left the forwarded set at step 1 (16 tier-1 + 7 tier-2), and the tooltip left it at step 2 part 3, taking 4 curated props and 24 measured passthrough rows with it. Both deltas are exact, which is the check that a section MOVED rather than being rewritten — `scripts/__tests__/wrapper-prop-reference.contract.test.js` pins all four numbers.

  **The erosion guard** is an `eslintConfig.overrides` entry in `package.json` — `files: src/**/*.{js,jsx,ts,tsx}`, `excludedFiles: src/core/components/**` — with `patterns: ["semantic-ui-react/*"]` alongside the `paths` entry, because `paths` alone does not stop a deep import such as `semantic-ui-react/dist/commonjs/…`. The glob lists `ts`/`tsx` for when §9.6-E2 converts `components`/`modules`, but that half is **inert today**: `lint:js` runs `eslint src --ext .js,.jsx`, so ESLint never visits a TS file. Until that script is widened, TS coverage is carried entirely by the generator's scan, which walks `.[jt]sx?`. Widening `--ext` needs a TS-aware parser and belongs with E2, not here. It fails `npm run lint:js` (`--max-warnings 0`) and was verified in both directions: a violating import in each of `core/utils`, `core/modules`, `demo` and `library` is reported, a file inside the pack (including a nested directory) is not, and the three real wrapper imports keep lint green. **Known blind spot, measured rather than assumed:** `no-restricted-imports` sees only a static `import`. It cannot see `jest.mock('semantic-ui-react')`, `require()`, or a **dynamic `import('semantic-ui-react')`** — ESLint 8 does not visit `ImportExpression` at all. Review found the dynamic-import case escaping *both* layers, so the generator's scan now matches it too, along with double-quoted specifiers, and walks TS files: probed in both directions, each escape now exits 1 with the offending file named. That is why both halves run — neither is sufficient alone.

  ```jsonc
  // package.json → eslintConfig.overrides[] — all of src except src/core/components
  "no-restricted-imports": ["error", {
    "paths": [{ "name": "semantic-ui-react", "message": "…only inside src/core/components…" }],
    "patterns": ["semantic-ui-react/*"]
  }]
  ```

- **Step 1 — `Table` — SHIPPED.** Native `<table>/<thead>/<tbody>/<tfoot>/<tr>/<th>/<td>`: seven components, ~119 lines, no `semantic-ui-react` import, no new dependency. The subcomponent API (`Table.Header`, `Table.Cell`, …) is unchanged, so none of the four call sites moved. Removes the 4 KB SUIR table code path; `Table.js` is no longer one of the SUIR import sites, and the derived inventory on `docs/SUPPORTED-PROPS.md` dropped from 7 to 6.

  **What the step-0 estimate got right.** The S held, at the small end. "Markup/className sugar only" was exactly right — all six subcomponents turned out to be the *same* function with a different tag, so they share one implementation. The four named obligations were the whole of the work: `ui table` on the root, `omitProps` in the cell, the `verticalAlign` decision, the coverage threshold. And the prediction that the DOM would change *deliberately* was right in shape and in size: **332 changed lines, in four shapes and no others** — 24 `<tbody class="">`, 94 `<tr class="">` and 199 `<td class="">` losing an empty attribute, and 15 `<td class="top aligned">` losing a dead class. Element counts per tag, every other class string, visible text, and every `id`/`style`/`colspan`/`name` value are byte-identical; the snapshot file did not change length.

  **What it got wrong, or under-stated.** (a) The tier-1 surface was published as "5 prop names across 7 components" including `verticalAlign` — but `verticalAlign` was tier-1 only because *our own* `LocalDraftTableRow` passed it, never because a meta did, and it did nothing. The real exercised surface is four names (`children`, `className`, `style`, `colSpan`), and "tier 1" needs reading as "something passes it", not "something depends on it". (b) `fixedHeader` was described as unused; it is also **non-functional** — both classes it emits have zero occurrences in `static/all.css` and in `src/style`, so it rendered two unstyled `<div>`s. Deleted with its test rather than reimplemented. (c) The step notes did not anticipate that removing the wrapper would require restructuring the `docs:props` generator: `Table` cannot stay in a `WRAPPERS` list that asserts a SUIR import, so the page now has two kinds of section (in-house vs wrapper) and the in-house half derives the element per subcomponent, the shared destructure, and the *absence* of any SUIR reference. That was the largest single piece of work in the step, and it is worth budgeting for again at steps 2 and 3.

  **Decisions recorded.** `verticalAlign` DROPPED (SUIR emitted `top aligned`; no loaded CSS selects on `aligned`, and no `.top` rule can match a cell, so the 15 cells that asked for it already rendered at the `<td>` default — pixel-identical, and both call sites went with it). `inverted`/`striped` KEPT (the only modifiers any call site passes, and both genuinely styled). `fixedHeader` DELETED. `as`/`celled`/`textAlign` DROPPED. `TableView.js`'s `sellStyles` typo resolved. Semver: on this evidence the step is **minor** — every dropped prop is either unused, unstyled, or both, and every prop any meta in either corpus passes still works; Step 5 records the call for the whole exit.

  **Gates.** The markup-independent behavioural layer was run BEFORE the snapshots were regenerated and was green on the new markup while the old baseline was still red: `examples.behavior-contract` + `UIRender.table-behavior` + `UIRender.listbox-behavior` + `UIRender.overlay-behavior`, 4 suites / 104 tests. Then one `-u`, then all three React legs at identical totals (158 suites / 2396 tests / 38 snapshots on 16.14, 17 and 18.3), per-file coverage (`Table.js` at 100/100/100/100, its own new threshold), `lint:js`, `lint:css`, `docs:views:check`, `docs:props:check`. The DOM zero-tripwires (`FIXED_PROP_LEAKS`, `FIXED_MARKUP_JUNK`) stayed at zero and the `name`-only-on-a-form-control invariant stayed at 57. **Still owed and not delivered by this step:** the mandatory Playwright visual/keyboard/a11y suite (§9.7-F1.3) does not exist in the repo, so the `.ui.table` padding contract is verified here by reading the compiled CSS and by class-string assertions, not by rendered pixels.

- **Step 2 — `TooltipPop` (S–M). SPLIT IN THREE, AND ALL THREE HAVE SHIPPED.** In-house tooltip: CSS placement + hover/focus triggers with delay + `aria-describedby`. Part 1 was the gate and the measurement, part 2 the browser leg, part 3 the replacement.

  **Why it was split.** The replacement could not be judged, because the component had no gate that a rewrite could fail. `TooltipPop.test.js` mocked the whole `semantic-ui-react` module and asserted the props object handed to the mock — assertions that pass against a `Popup` rendering nothing, describing the seam this step deletes. Layer 1 cannot see the component at all, and not merely because the corpus's two declarations sit in an inactive `Tabs` panel: a closed tooltip adds **nothing** to the document, so even an active one would leave all 38 snapshots unchanged. Part 1 was therefore measurement plus gate, product code untouched; part 2 is the replacement. Sequencing lesson worth reusing at step 3: measure and gate in a separate PR from the swap, because the measurement changed what the swap is *for*.

  **Part 1 — SHIPPED (tests and docs only).** 63 tooltip tests across four files where there were 10 — and only 5 of those 10 could fail if the tooltip broke, because the other 5 asserted the props handed to a mock:
  - `components/__tests__/TooltipPop.test.js` — **rewritten against the real `semantic-ui-react`**, no `jest.mock`. The emitted DOM, the exact class string with the CSS rule each token feeds, the portal's mount parent, the `title` shapes (string/number/element/function/`header`), the passthrough surface, and the a11y and prop-leak tripwires.
  - `components/__tests__/TooltipPop.behavior.test.js` — new. What opens and closes it, in milliseconds, as behaviour rather than markup.
  - `pages/main/__tests__/UIRender.overlay-behavior.test.js` — extended from one meta entry point to all three (`view: "Tooltip"`, a string `tooltip` attribute, an object `tooltip`), plus the click path and the `items` failure mode.
  - `style/__tests__/css.tooltip-contract.test.js` — new. Compiles the real CSS through the real webpack PostCSS stage, renders the real open tooltip, and runs `Element.matches()` rule by rule. This is the join neither the markup tests nor the CSS tests could see.

  **Four corrections to what this step believed.**
  1. It was not ungated: `UIRender.overlay-behavior.test.js` already drove the real component through the real engine for 5 clauses. What was missing was everything markup-shaped and every dismissal path — no class pin, no portal-location pin, no click, no Escape, no click-outside, no close delay, no object-`tooltip` passthrough, no prop-leak boundary.
  2. "The corpus renders zero tooltips" is true of **this** component only. The corpus renders **5**, all in the `slider` example and all snapshot-gated, from a *second* tooltip: `components/Tooltip.js`, 15 lines, CSS-only, an inline `<span>`, no portal and no JS positioning, used by `Slider`, `modules/upload/views/Upload.js` and the form validation-error tooltip, styled by 41 rules in `style/components/tooltip.less`. It is both evidence for the positioning decision and a naming trap for the cleanup — `form/utils.js` imports **both**.
  3. **"Keep emitting `ui popup`-compatible classNames until Step 4 so the current CSS continues to apply" rests on a false premise: no `.ui.popup` rule applies today.** The 13 declaration blocks that select this exact class string (plus 6 placement-keyed `:before` rules for the arrow) are all `.ui-render`-scoped by prefixwrap; `.ui-render` is a `<div>` and SUIR's `PortalInner` mounts into `document.body`, outside it. Measured twice — by selector matching in jsdom and by computed style in real Chrome, where the bubble comes out with `background: rgba(0,0,0,0)`, no border, no padding, `max-width: none`, `z-index: auto` and `::before { content: none }`, i.e. bare text overlapping the trigger. The class contract becomes real the moment the replacement mounts inside the widget, and SUIR's own `mountNode` already shows that it can.
  4. The largest coverage lesson: `TooltipPop.js` was already at **100% statements / branches / functions / lines** before part 1. Line coverage was never the gate and a per-file threshold would not have been one either.

  **Part 2 — SHIPPED (the browser leg).** 38 Playwright tests over Chromium (39 after part 3 added the out-of-flow limitation site): a corpus spec (every tooltip a meta can declare), a purpose-built harness page (`src/demo/pages/TooltipHarness.jsx`) for the cases the corpus cannot express, a `hasTouch` project, and a keyboard/a11y spec whose helpers step 3's `Dropdown` matrix reuses. Every expected value lives in `e2e/reference.js` tagged `[I]` invariant / `[R]` current behaviour / `[R->I]` pinned defect, so a value that must change when the component changes says so. That tagging is what made part 3 tractable: 23 of the 38 tests were `[R]`/`[R->I]` facts about the wrapper, and each had to be re-measured and rewritten rather than deleted.

  **Part 3 — SHIPPED (the replacement).** `TooltipPop.js` no longer imports `semantic-ui-react`. It renders `<span class="tooltip-host">` around the trigger plus a `Tooltip` bubble — the same 15-line inline `<span>` the product has shipped for years through `components/Tooltip.js` — mounted **only while open**, placed by CSS in `tooltip.less`. Mount-on-open is the crux rather than an implementation detail: a bubble that is in the DOM is revealed instantly by `*:hover > .tooltip` and no class defeats that at equal specificity, so mount-on-open is what buys the 500 ms delay, focus-open, Escape and click-outside at once.

  **It was a fix, not a trade.** Measured in real Chrome on the production build at every use site a meta can declare: the SUIR bubble rendered at the document origin — ~730 px from its trigger on `buttonIcon`, 2538–3006 px at the deep `all` site — with no `data-popper-placement` anywhere, and **every open raised an uncaught `TypeError`** from popper's flip modifier. Cause: SUIR clones the trigger with a `ref`, and of everything a meta can declare nothing can hold one (`mapper.js` uses the plain `Row`; `Button` is a `React.memo(function Button)`), so popper's reference element was `null` and `getClippingParents(null)` threw before a coordinate was written. On top of that the bubble portaled into `document.body`, outside `.ui-render`, so prefixwrap left all 13 `.ui.popup` rules unable to match and the live tooltip was unstyled text. There was no working positioning to lose. Measured now: 40 px centre-to-centre at **both** sites, painted, and nothing thrown.

  **Positioning primitive — DECIDED: (b), zero-dep, CSS off the host box.** `@floating-ui/dom` was not needed once the measurement showed there was no positional behaviour to match, and CSS anchor positioning stays rejected (not Baseline). The costs are named where they are measured, not summarised away: no flip, no viewport shift, no 250 px wrap, the bubble clipped by an `overflow: hidden` ancestor, and a trigger taken **out of normal flow** leaves the host collapsed so the bubble follows the host (its own harness section). The first three were reachable **only** on the harness page — from a meta, popper threw before writing a coordinate, so nothing in the product ever flipped. Step 3's option list therefore does **not** inherit a shared positioning primitive from this step; it makes its own call.

  **Decisions, all four discharged.** `omitProps` applied — `view`, `index` and `symbol` no longer reach the bubble as HTML attributes. `on` **dropped**, on the maintainers' instruction, and recorded in `docs/SUPPORTED-PROPS.md` under `dropped.on` since the CHANGELOG is step 5's debt; focus-open, `role="tooltip"`, the bubble `id` and `aria-describedby` came in as a *consequence*, because with click gone and hover unavailable to a keyboard there would be no path to the content at all. Passthrough narrowed from **45 reachable names to 13**, with the 18 that were reachable-and-plausible warning once each in development instead of being silently ignored. Trigger shape: `React.Children.only` is gone with the portal, so the `items` form of `view: "Tooltip"` now renders instead of throwing the engine's diagnostic. The dead `TooltipPop` chain in `modules/form/utils.js` is deleted at all four sites, line 7 untouched.

  **Two defects the browser leg caught that no jsdom suite could, and this is the case for the leg existing.** (1) Dropping the click gesture changed nothing a user sees, because clicking a `<button>` **focuses** it and focus-open then showed the bubble instantly — and a touch **tap** synthesises the compatibility mouse sequence with nothing following to move the pointer away, so the emulated hover stuck and the bubble appeared 500 ms after every tap. Both are suppressed by reading the pointer (`onPointerDown`, `event.pointerType`). Neither could have been *found* in jsdom — nothing there focuses on click, and a tap has no compatibility mouse sequence — but both are pinned there now; `pointerType` needs a hand-built event to survive, because jsdom implements no `PointerEvent` and RTL's `fireEvent.pointerOver(el, {pointerType})` therefore delivers `null` (measured, and the reason a first attempt read as unassertable). (2) With the component correct the bubble still landed **559 px** from its trigger: `.tooltip-host` is `inline-flex`, but a flex item is blockified to `flex` and then stretched by `align-items: stretch`, so inside `div.flex--col` the host came out 1222 px wide around a 35 px button. Fixed with `width: fit-content` (a definite cross size, so `stretch` stops applying); `width` only, because `height: fit-content` would also stop the host stretching vertically in a row container, which is how a wrapped button gets its height today. **Every jsdom suite was green while the shipped tooltip was unusable.**

  **Finding 4 closed, and half of it was never broken.** `tooltip.less`'s four corner placements did put the bubble on its own trigger — `.tooltip.left`/`.right` set `top: 50%` at the same specificity as `.top`/`.bottom`, so a corner class string matched both and the axis came out over-constrained. Fixed by writing the losing offset back per corner rather than by raising specificity. A **second, independent** defect had to go with it: the verdict required every corner to align to its host's *left* edge, which `top right` and `bottom right` cannot do by definition, so those two would have stayed red after the CSS was right. All four were genuinely broken in CSS — `PLACEMENTS_OVERLAPPING_HOST` recorded both top corners sitting on their own host — and an earlier draft of this section wrongly said the right corners had been placing correctly all along. 8 of 8 now. The component asks for `top`, not the wrapper's `top left` — that was SUIR's own default rather than a meta's request, so the bubble is centred above its trigger instead of left-aligned above it.

  **The accessibility trade, stated rather than sold.** 1.4.13 wants hovered content hoverable; this bubble keeps `pointer-events: none` because without it the bubble sits under the pointer, `mouseleave` fires on the host and the tooltip flickers — so the pointer over the bubble is really over what is behind it and it closes, exactly as the wrapper behaved without `hoverable`. Hoverable text and a non-interactive bubble are mutually exclusive and the bubble stays non-interactive. And a **touch-only device now has no way to see a tooltip at all**: the same trade desktop makes (the tooltip is supplementary, the node's action is not), and the reason the content must never be the only place information lives. `pointer-events: none` also invalidated an *instrument* — `document.elementFromPoint` skips such elements, so every "is the bubble painted here" assertion silently inverted; `e2e/fixtures.js` grew `paintedTopmostAt`, which re-enables pointer events for the probe alone.

  **An adversarial review after the step was green found three real bugs, and that is the lesson worth carrying to step 3.** Six independent lenses over the change set, each finding then put to three skeptics told to refute it: 55 candidates, 13 upheld, 8 refuted — and 34 left with **no verdict at all** because the verifying agents hit a session limit, which is recorded here because "not verified" is not "refuted" and the difference is the whole value of the panel. The three code defects, none of which any gate could see:
  - **`aria-describedby` was ASSIGNED, not appended.** A trigger can already point at something of its own — a form control at its validation message is the case in this product — so the error text was unlinked for exactly as long as the tooltip was open: present while a sighted user reads the tooltip, absent from the announcement that matters. Now appended, trigger's own ids first, because order is announcement order.
  - **The pointer guard outlived its interaction.** `fromPointer` was cleared only on blur, so a `pointerdown` that moved no focus left it standing for the component's life and suppressed the next KEYBOARD focus — the guard protecting the click removal was disabling the keyboard path the same removal created. Now one-shot, and cleared when the pointer leaves.
  - **A controlled `open={true}` never reported its first close.** `reported` starts `false` (right for an uncontrolled tooltip, which starts closed), so a tooltip mounted open took the early return in `change()` and a host trusting `onClose` stayed open for good. `reported` now tracks the effective state.

  Each fix is pinned by a test **verified to fail when the fix is reverted** — checked one at a time, because a regression test nobody has seen fail is a guess. The review also caught eight prose defects in this very change set, including two claims of my own that measurement refuted: that the right corner placements "were placing correctly all along" (they were not — both top corners sat on their own host, and the assertion defect was a *second*, independent problem) and that "no class can defeat" the hover reveal at equal specificity (`&.show { animation-delay: 0s }` four lines later does exactly that; the real reason mount-on-open is necessary is that the reveal has no delay to inherit).

  **Gates.** jsdom: 160 suites / 2467 tests / 38 snapshots. Chrome: 39/39 across four specs and two projects. `docs:props:check` green with `TooltipPop` moved from `WRAPPERS` to `IN_HOUSE` in the generator — which required making `subcomponents` and the shared `Part` reader optional, since the in-house shape had assumed a component family (`Table`) and this one is a single element. One `-u` on the 38-example baseline: the `.tooltip-host` wrapper is new markup at tooltip sites, 10 lines of snapshot diff.

- **Step 3 — `Dropdown` (L, ~2–4 weeks incl. regression).** Step 0 confirmed the L and relocated it: not `search`/`multiple`/`allowAdditions`/`clearable` (unused everywhere), but the keyboard/a11y matrix, the cascading flows, `upward`'s auto-flip (an autoControlled prop — left unset SUIR measures viewport space and flips itself, so the replacement owes the behaviour as well as the prop) and the `.ui.selection.dropdown` CSS contract, which couples this step to Step 4 more tightly than the sequencing implies. Two viable paths:
  ...
  The wrapper's own logic (sanitization, dedup, additions handling, cascading reset, `onChange` signature) is **kept as-is** — only the `<DropDown …/>` element at the bottom is replaced. One extra contract discovered by the re-audit: the form adapter special-cases `InputComponent.displayName === 'Dropdown'` (`form/utils.js:193,199`) — the replacement must keep `displayName = 'Dropdown'`, or that adapter branch is refactored in the same PR. **Step 0 sharpened it:** the branch works only because `DropdownField.js` imports the *named* export, since `React.memo(...)` does not carry `displayName` (the default export has none). The named-vs-default split is therefore part of the contract, not an implementation detail, and `scripts/__tests__/wrapper-prop-reference.contract.test.js` now pins both facts. Gate: the behavioral layer of the §9.5 suite + the mandatory visual/keyboard/a11y suite + full example QA (full-DOM snapshots are expected to change here — regenerate deliberately); the cascading-Select flows driven from `rules.js`/`mapper.js` are the regression hotspot.
...
- **Step 5 — cleanup + release.** The `semantic-ui-react` dependency is already gone (Step 3½); this step drops `semantic-ui-less`, finishes changelog + supported-prop documentation, and records the bundle delta (−30 KB+ per the source's own estimates: 27 KB Dropdown + 4 KB Table + Popup). **Step 0 found no unsupported passthrough props in the wild**, so on current evidence this is a **minor** release — every forwarded prop in either corpus is on the tier-1 list and is meant to keep working. The tier-2 decision is what could still turn it major: dropping a published propType is a breaking change even when nothing observed uses it, so record that call explicitly in the step PR rather than inheriting it.

##### F1.3 Sequencing & effort

Run after Phase 2 (React 18) and Phase 3 (contract tests — they are the safety net). Steps 1–2 can proceed in parallel with Workstream A; Step 3 deserves dedicated focus. Every step is gated by the §9.5 behavioral layer **plus the mandatory visual/keyboard/a11y suite**, which **now exists** (`playwright.config.js` + `e2e/`, shipped by Step 2 part 2, gating CI job `browser`). Step 1 shipped before it, against the behavioural layer plus CSS reading, and is **not** retro-credited: its `.ui.table` padding contract stays verified by reading compiled CSS. Step 2 is what forced the suite into existence — the tooltip's whole purpose is positional, part 1 established that no jsdom suite can express it, and part 2 could not be called done on jest alone. Step 3 inherits the harness, the keyboard/a11y probes and a measured `Dropdown` starting state (`e2e/reference.js` → `DROPDOWN`), which is the expensive half of its matrix, but the matrix itself is still Step 3's work. Step 2 also demonstrated a sequencing improvement worth reusing at step 3 — **measure and gate in one PR, swap in the next** — because the measurement changed what the swap is for (see corrections 2 and 3 there). Total: **~4–7 weeks** spread across independently shippable releases — Step 0's audit left every per-step estimate unchanged (S / S–M / L), and Step 1 came in at the small end of its S, so the schedule stands; what Step 0 changed is *where* the effort sits in Steps 2 and 3 and what each step's definition of done includes. One cost Step 1 surfaced that the estimates do not carry: taking a wrapper in-house also means restructuring the generated prop page, because a component that imports nothing cannot stay in a list whose entries are asserted to import SUIR. Completing 5a — including Step 3½ — removes the React 19 external blocker (§8).

One measurement worth carrying into Step 4's estimate: the five loaded semantic-ui-less modules compile to roughly **45% of the final CSS payload**, dominated by `modules/dropdown` and `collections/menu`. Step 4 is re-homing that, not trimming an edge.
...
| **5a** | **SUIR exit, JS side (F1 steps 1–3½): ~~`Table`~~ **shipped** → `TooltipPop` → `Dropdown`, then `semantic-ui-react` removed from `dependencies`**, shipped step by step | ~3–5 weeks | behavioral + visual/a11y suites per step |
...
| R2 | `semantic-ui-react` upstream becomes fully unmaintained before the exit completes (stuck on an unpatchable beta) | Medium | Medium | Planned full exit (§9.7-F1) caps the exposure; isolation + eslint guard until then (**both in place since step 0**); a 3-component surface keeps even an emergency exit bounded |
...
# SUIR erosion guard + published prop surface (§9.7-F1 step 0)
# The eslint override fails lint:js on an `import` outside src/core/components; the generated
# page's own scan additionally catches `require()` and `jest.mock()`, which the rule cannot see.
npm run lint:js
...
- ☑ **Step 0:** passthrough-prop audit across example **and consumer** metas → published parity checklist at `docs/SUPPORTED-PROPS.md`, generated from the component source and drift-checked in CI and in the suite. Result at the time: 37 tier-1 (the three wrappers + the six `Table` subcomponents) (exercised) and 24 tier-2 (published, unexercised) forwarded props; `search`/`multiple`/`allowAdditions`/`clearable` are tier 2, `upward`/`disabled`/`colGroup` were found only in consumer metas. **Now 21 / 17**, because step 1 took the whole `Table` family (16 tier-1 + 7 tier-2) out of the forwarded set — a component that imports nothing forwards nothing; those props are on the page as *dropped* or as passthrough instead
- ☑ **Step 0:** `no-restricted-imports` guard on `semantic-ui-react` active outside the components pack (`package.json` → `eslintConfig.overrides`, with `patterns` for deep imports), verified to fail `lint:js` on a violation in each non-pack layer and to stay silent inside the pack
- ☑ **Step 0:** the `displayName`/named-export contract (`form/utils.js:193,199`) pinned by a contract test, so Step 3 cannot break it silently
- ☑ **Step 1:** `Table` in-house — native `<table>/<thead>/<tbody>/<tfoot>/<tr>/<th>/<td>`, seven components, subcomponent API unchanged, `Table.js` no longer imports `semantic-ui-react` (the derived import inventory dropped 7 → 6)
- ☑ **Step 1:** the behavioural layer was run BEFORE regenerating snapshots and was green on the new markup while the old baseline was still red (4 suites / 104 tests); the 332 changed snapshot lines are four shapes and no others, and every other line, every element count per tag and every `id`/`style`/`colspan`/`name` value is byte-identical
- ☑ **Step 1:** the root still emits `ui table`, so the in-house cell padding (`.ui.table td > :not(.button)`) keeps applying; all 24 `<table>` class strings unchanged, pinned by a unit test as well as by the baseline
- ☑ **Step 1:** `omitProps` applied in all seven components, closing the `mapper.js:184` spread onto `Table.Cell`; `domProps.js`'s "WHERE THE LISTS ARE APPLIED" note names the family
- ☑ **Step 1:** decisions recorded in the step PR and on `docs/SUPPORTED-PROPS.md` — `verticalAlign`/`as`/`celled`/`textAlign` dropped, `inverted`/`striped` kept, `fixedHeader` deleted (unused *and* non-functional), semver call **minor**
- ☑ **Step 1:** `jest.config.js` per-file threshold for `Table.js` at 100/100/100/100, measured
- ☑ **Step 2 part 1:** the gate exists before the swap — 63 tooltip tests in four suites, up from 5 that could fail if the tooltip broke (`TooltipPop.test.js` rewritten against the REAL `semantic-ui-react`, `TooltipPop.behavior.test.js`, `UIRender.overlay-behavior.test.js` extended to all three meta entry points, `css.tooltip-contract.test.js` joining the emitted class string to the compiled CSS). Product code untouched
- ☑ **Step 2 part 1:** the className contract pinned as a literal (`ui top left inverted popup transition visible`) with the 13 scoped CSS rules and 6 arrow rules it feeds named against it — part 3 replaced that literal with `tooltip no-wrap top show inverted`, and the CSS join moved to `tooltip.less`, which the bubble can now actually reach
- ☑ **Step 2 part 1:** the interaction contract pinned as behaviour and exact milliseconds — 500 open / 70 close, leave-before-500 cancels, click opens instantly, second click / click-outside / Escape close, click inside does not, pointer-onto-bubble still closes. Part 3 kept the milliseconds, the cancel, click-outside, Escape and pointer-onto-bubble, and dropped the click gesture
- ☑ **Step 2 part 1:** four corrections recorded — the component was not ungated; the corpus's 5 rendered tooltips belong to the *other*, in-house `Tooltip.js`; **no `.ui.popup` rule applies today**, so "keep emitting `ui popup` classNames" preserves nothing; and `TooltipPop.js` was already at 100/100/100/100 coverage, so line coverage was never the gate
- ☑ **Step 2 part 2 (the browser gate, shipped before the swap):** `playwright.config.js` + `e2e/` — 4 spec files / 39 tests over one fixture module and one reference ledger, `@playwright/test` pinned exactly in `devDependencies`, gating CI job `browser` in parallel with `verify`. Serves a production build (`PUBLIC_PATH`/`OUTPUT_DIR` overrides, both defaulting to today's values) because the dev server's error overlay intercepts every interaction once a tooltip throws. `npx jest` unchanged at 160 / 2451 / 38 on all three React legs
- ☑ **Step 2 part 2:** a `TooltipPop` is observable without activating the inactive `Tabs` branch and without adding an example — the existing `Factors` branch is driven as-is (hash-expand + one tab click, the deep site with ten clip/scroll ancestors) plus one measured-free `tooltip:` line on `button-icon_meta.js`. Activating the branch (~240 snapshot lines, `ROLE_CENSUS.all`) and adding an example (nine enforced touchpoints) were costed and rejected
- ☑ **Step 2 part 2:** every browser-only fact recorded in `e2e/reference.js` tagged `[R]` reference / `[I]` invariant / `[R->I]` pinned defect, so the suite is green today, gates today, and the diff that flips a pin is the record that the swap fixed something — not a tripwire on the plan
- ☑ **Step 2 part 2:** five findings, four of them corrections to this document — (1) the shipped tooltip is **unpositioned**, not merely unstyled, and throws once per open because nothing a meta can declare can hold a `ref`; (2) flip also flips the alignment, so horizontal overflow *is* handled, and no `data-popper-*` attribute exists; (3) any scroll closes the bubble via SUIR's unconditional `hideOnScroll`, so scroll repositioning is unreachable rather than invisible; (4) **only four of `tooltip.less`'s eight placements work and `top left` is not one of them**, which is a new obligation for part 2; (5) clipping demonstrated in both directions, `position: fixed` inside `.ui-render` verified to escape `.app`'s clip, and `.ui-render` verified to create no stacking context
- ☑ **Step 2 part 3:** the popper crash is **gone with the code path**, which is why part 2 pinned it rather than fixing it with a `forwardRef`. Measured: 0 page errors per open at both use sites, against 1 uncaught `TypeError` per open before
- ☑ **Step 2 part 3:** the replacement shipped, and the positioning primitive **decided: zero-dep CSS placement off the host box**, no library. Justified by the measurement rather than by taste — on every meta path popper wrote no coordinates at all, so there was no positional behaviour to be a regression against. Costs named where they are measured: no flip, no viewport shift, no 250 px wrap, clipping by an `overflow: hidden` ancestor, and an out-of-flow trigger leaving the host collapsed (its own harness section). Step 3 inherits no primitive from this step and makes its own call
- ☑ **Step 2 part 3:** `TooltipPop.js` imports no `semantic-ui-react`; the derived import inventory dropped 6 → 3, and the 3 that remain are `Dropdown.js` plus its behaviour test's pair. Passthrough narrowed from **45 reachable names to 13**; the 18 reachable-and-plausible ones warn once each in development instead of being ignored silently
- ☑ **Step 2 part 3:** `on` **dropped** on the maintainers' instruction, recorded under `dropped.on` in `docs/SUPPORTED-PROPS.md` (the CHANGELOG itself is step 5's debt), with focus-open + `role="tooltip"` + bubble `id` + `aria-describedby` added as its consequence — with click gone and hover unavailable to a keyboard there would be no path to the content at all
- ☑ **Step 2 part 3:** the removal was made REAL by two guards the browser leg found, neither of them findable in jsdom — clicking a `<button>` focuses it (so focus-open reopened the collision instantly), and a touch tap's compatibility mouse sequence leaves a sticky emulated hover (so the bubble appeared 500 ms after every tap). Suppressed via `onPointerDown` and `event.pointerType`, and pinned in **both** legs — in jsdom the pointer event has to be hand-built, because RTL falls back to `MouseEvent` and drops `pointerType` (delivering `null`), which is why a first attempt read as unassertable
- ☑ **Step 2 part 3:** a **second positioning defect** found and fixed with the component already correct — `.tooltip-host` is `inline-flex`, but a flex item is blockified to `flex` and stretched by `align-items: stretch`, so the host came out 1222 px wide around a 35 px button and the bubble sat 559 px away. `width: fit-content` fixes it; `width` only, deliberately. Every jsdom suite was green while the shipped tooltip was unusable
- ☑ **Step 2 part 3:** **finding 4 closed** — the four corner placements fixed in `tooltip.less` by writing the losing offset back per corner, plus a second, independent fix to the verdict, which required every corner to align to its host's *left* edge and so would have kept `top right` and `bottom right` red after the CSS was right. All four were genuinely broken in CSS; the assertion defect affected two of them. 8 of 8 placements now
- ☑ **Step 2 part 3:** 23 of the 38 browser tests were `[R]`/`[R->I]` facts about the wrapper and were **re-measured and rewritten**, not deleted — including one that was passing for the wrong reason (an "absence" assertion counting the semantic-ui-react portal selector, which matches nothing once the tooltip is in-house, so "focus does not open it" stayed green after focus started opening it)
- ☑ **Step 2 part 3:** gates — jsdom 160 suites / 2467 tests / 38 snapshots, Chrome 39/39, `docs:props:check` green with `TooltipPop` moved to `IN_HOUSE` (which required making the generator's `subcomponents` and shared-`Part` readers optional: the in-house shape had assumed a component family). One `-u`: the `.tooltip-host` wrapper is new markup at tooltip sites, 10 snapshot lines
- ☑ **Step 2 part 3:** an adversarial review AFTER the step was green found three real bugs no gate could see — `aria-describedby` assigned instead of appended (unlinking a form control's own validation message for as long as the tooltip is open), the pointer guard outliving its interaction (a `pointerdown` that moved no focus suppressed the next keyboard focus, i.e. the guard disabled the path the removal created), and a controlled `open={true}` never reporting its first close. Each fix is pinned by a test **verified to fail when the fix is reverted**, one at a time
- ☐ **Step 2 part 3:** 34 of the review's 55 candidate findings have **no verdict** — their verifying agents hit a session limit, and "not verified" is not "refuted". The titles are in the run's journal; the ones worth re-running before step 3 are the `uncovered-behaviour` set (controlled-mode edge cases, unmount-while-open, a function `title` re-invoked per render) and the `stale-wrapper-references` set (headers in five spec files still explain themselves by the wrapper they no longer test)
- ☐ **Step 2 part 3:** the accessibility trade is **recorded, not resolved** — 1.4.13 wants hovered content hoverable and this bubble cannot be, because `pointer-events: none` is what stops it swallowing the pointer and flickering; and a touch-only device now has no way to see a tooltip at all. Both are consequences of decisions the maintainers took, and both are written down in `docs/SUPPORTED-PROPS.md`, `e2e/reference.js` and §9.7-F1 step 2 rather than left for a reader to discover
- ☐ **Step 2 part 3 — OBLIGATION 1: the 500 ms open delay survives.** The inline component reveals through the CSS rule `*:hover > &` (`style/components/tooltip.less`), which has no delay, so a naive convergence drops it. It is a deliberate UX decision, stated in `TooltipPop.js` — "avoiding an accidental popup when the user is already familiar with the UI" — and pinned to the millisecond in three places (`[I] hover opens after our 500 ms delay` in `e2e/corpus.tooltip.pw.js`, `TooltipPop.behavior.test.js`, and the 499/500 boundary in `UIRender.overlay-behavior.test.js`). Measured: the simulated convergence fails all of them. Keep it in JavaScript, or `transition-delay` if the reveal stays CSS — either way it must remain gated
- ☐ **Step 2 part 3 — OBLIGATION 2: click-to-open survives.** SUIR's `on: ['click', 'hover']` gives the tooltip a click path the CSS rule has no equivalent for. Measured as failing under the simulated convergence (`[R] click opens with no delay, a second click closes`, `e2e/harness.tooltip.pw.js`). Decide deliberately: reimplement, or drop it and say so in the changelog — but not by omission
...
- ☐ **Step 2 part 3 — HAZARD: the bubble must not intercept pointer events.** An inline bubble positioned over its trigger swallows the pointer, so the trigger cannot be hovered — Playwright reported `<span class="tooltip …"> intercepts pointer events` and retried until timeout. `pointer-events: none` is therefore a requirement of the inline shape. `tooltip.less:12` hints at the same hazard from the other direction (`display: none` is used because `visibility: hidden` "will trigger hover"), and the reason `Slider`/`Upload` never hit it is that their bubbles do not cover an active area. Adding it fixed exactly one of the five simulated failures, so it is necessary and not sufficient
- ☐ **Step 2 part 2 (NEW, from the browser leg):** `tooltip.less`'s corner placements fixed, or the vocabulary explicitly restricted — `.tooltip.left`/`.tooltip.right` set `top: 50%` at the same specificity as `.tooltip.top`/`.tooltip.bottom`, so `top left`/`top right` land ON the host and `bottom left`/`bottom right` degrade to `left`/`right`. `top left` is the only placement `TooltipPop` uses, so converging on the in-house `Tooltip` without this ships a bubble sitting on its own trigger
- ☐ **Step 2 part 2:** `omitProps` applied (the boundary leaks `view`/`index`/`symbol` onto the bubble today, pinned as a defect); `on` decided; the 45-prop passthrough decided; the `items` trigger shape decided; the dead 4-site `TooltipPop` chain in `modules/form/utils.js` deleted
- ☐ **Steps 2–3:** per component — behavioral contract layer + example QA green (full-DOM snapshots regenerated deliberately); emitted classNames keep existing LESS working
...
- 🔶 Visual/keyboard/a11y suite green per step (mandatory, §9.5) — **the suite now EXISTS** (`playwright.config.js` + `e2e/`, gating CI job `browser`), shipped by Step 2 part 2 and **built for Step 2**. Green for Step 2: all ten named jsdom gaps closed except real assistive technology, which no browser runner can close. **Still NOT met for Step 1** and deliberately not retro-credited — its `.ui.table` padding contract was verified by reading the compiled CSS and by class-string assertions, not by rendered pixels, and stays that way. **Not met for Step 3**, which owns its own `Dropdown` keyboard/a11y matrix; what it inherits is the harness, the probes and a measured starting state (`e2e/reference.js` → `DROPDOWN`: role census, `aria-expanded` behaviour, options present when closed, and the five missing combobox ARIA attributes as a defect inventory). What the suite can never see is enumerated in §9.5 — real AT, cross-browser, real OS pointer/touch, pixels, the published artifact, the 16/17 peer ends, and the `tabs`/`upload` keyboard contracts
