...
- ☑ **Step 2 part 3:** **finding 4 closed** — the four corner placements fixed in `tooltip.less` by writing the losing offset back per corner, and two of the four (`top right`, `bottom right`) turned out never to have been broken: the assertion required every corner to align to its host's *left* edge, which a right corner cannot do. 8 of 8 placements now
