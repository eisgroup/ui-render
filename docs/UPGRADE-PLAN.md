...
- **Step 2 — `TooltipPop` (S–M, ~3–5 days).** In-house tooltip: portal + positioning + hover/focus triggers with delay + `aria-describedby`. Positioning primitive options:
  ...
  Keep emitting `ui popup`-compatible classNames until Step 4 so the current CSS continues to apply. Step 0 moved the risk here without changing the size: **the tracked corpus renders zero tooltips** (both declarations sit in a non-active `Tabs` panel — neither trigger label nor any popup markup appears in the 38-snapshot baseline), so layer 1 cannot catch a tooltip regression at all and the gate is `UIRender.overlay-behavior.test.js` plus the wrapper unit test. Two decisions to make explicitly: SUIR's `on` defaults to `['click', 'hover']`, so today's tooltip *also opens on click* and does *not* open on focus; and option (b) above would now be a **regression** against today, because dropping SUIR also drops `@popperjs/core`/`react-popper` and with them flip and overflow handling.
...
- ☐ Visual/keyboard/a11y suite green per step (mandatory, §9.5) — **not met for Step 1**: the suite does not exist yet, so the `.ui.table` padding contract was verified by reading the compiled CSS and by class-string assertions rather than by rendered pixels. Carried as debt, not as done.
