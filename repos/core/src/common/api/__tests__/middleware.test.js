...
import { CREATE, DELETE, ERROR, GET, LIST, REQUEST, SUCCESS, TIMEOUT } from '../../constants'
import { isFunction, isList, toJSON } from '../../utils'
...
describe(`${API} middleware`, () => {
  ...
  ({serializer = null, payload = null, ACTION = GET, response = {id: 777}, status = 200, headers = {}}) {
    ...
    const finalUrl = (payload && payload.params) ? urlWithParams(API_TEST_URL, payload.params) : API_TEST_URL
    mock.reset()
    ...
    return store.dispatch(apiActionTest(API_TEST_URL, ACTION, payload, {authenticate: false, callRequest: true}))
      .then(() => {
        const calledActions = store.getActions()
        expect(calledActions.length).toBe(2)
        expect(calledActions[0].type).toEqual(apiActionType(API_TEST_URL, ACTION, REQUEST))
        expect(calledActions[0].payload.endpoint).toEqual(finalUrl)
        expect(calledActions[1].type).toEqual(apiActionType(API_TEST_URL, ACTION, SUCCESS))
  ...
  ({status, response, message = null, serializer = null, payload = null, ACTION = GET, headers = {}}) {
    ...
    /* Verify Error action */
    return store.dispatch(apiActionTest(API_TEST_URL, ACTION, payload, {authenticate: false}))
      .then(() => {
        const calledActions = store.getActions()
        expect(calledActions.length).toBe(1)
        expect(calledActions[0].type).toEqual(apiActionType(API_TEST_URL, ACTION, ERROR))
  ...
  it('returns meta data with headers and request data for successful response with params', () => {
    ...
    mock.reset()
    mock.onAny(urlWithParams(API_TEST_URL, params)).reply(
      200,
      toJSON(response),
    ...
    return store.dispatch(apiActionTest(API_TEST_URL, GET, {params}, {authenticate: false}))
      .then(() => {
        const calledActions = store.getActions()
        expect(calledActions.length).toBe(1)
        expect(calledActions[0].type).toEqual(apiActionType(API_TEST_URL, GET, SUCCESS))
        ...
        expect(calledActions[0].meta.request.url).toEqual(API_TEST_URL)
  ...
  it('calls API -> TIMEOUT for no responses', () => {
    ...
    const store = mockStore(initState)
    mock.reset()
    mock.onAny(API_TEST_URL).reply(() => new Promise((resolve) => setTimeout(() => resolve([200, {}]), 100)))

    /* Verify REQUEST and TIMEOUT actions are dispatched */
    return store.dispatch(apiActionTest(API_TEST_URL, GET, null, {timeout: 1, authenticate: false, callRequest: true}))
      .then(() => {
        const calledActions = store.getActions()
        expect(calledActions.length).toBe(2)
        expect(calledActions[0].type).toEqual(apiActionType(API_TEST_URL, GET, REQUEST))
        expect(calledActions[1].type).toEqual(apiActionType(API_TEST_URL, GET, TIMEOUT))
  ...
  it('calls NETWORK -> ERROR when connection fails', () => {
    ...
    mock.reset()
    mock.onAny(API_TEST_URL).reply(200, Promise.reject(error))

    /* Verify REQUEST and ERROR actions are dispatched */
    return store.dispatch(apiActionTest(API_TEST_URL, GET, null, {authenticate: false, callRequest: true}))
      .then(() => {
        const calledActions = store.getActions()
