...
export function getCookie (cookie, key) {
  ...
  let value = cookie.split('; ')
    .find(string => string.indexOf(key + '=') === 0)
  if (!value) return
...
isIpAddress.pattern = new RegExp('^(([0-9]|[1-9][0-9]|1[0-9]{2}|2[0-4][0-9]|25[0-5])\\.){3}' +
...
export function formatKeyPath (keyPath) {
  return keyPath.replace(/(\[)(.*?)(\])/g, (match, match1, match2, match3, offset) => (offset > 0 ? '.' : '') + String(match2))
}
